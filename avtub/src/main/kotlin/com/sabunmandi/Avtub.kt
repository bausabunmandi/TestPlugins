...
class Avtub : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster =
                videoPlayer.select("meta[itemprop=thumbnailUrl]").attr("content").trim().ifEmpty {
                    document.selectFirst(".featured-image img")?.attr("src")?.trim()
                }
    ...
    //         val unPacked = JsUnpacker(extractedPack).unpack() ?: extractedPack
    //
