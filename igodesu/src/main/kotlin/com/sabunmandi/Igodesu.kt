...
class Igodesu : MainAPI() {
        ...
        private suspend fun handleRandomPage(
                page: Int,
                ...
                val response =
                        app.post(
                                ...
                                headers =
                                        mapOf(
                                                "X-Requested-With" to
                                                        "XMLHttpRequest", // Required for AJAX
                                                "Content-Type" to
                ...
                val items =
                        document.select(".video-item").mapNotNull { item ->
                                // Your existing item parsing logic
                                val content =
                                        item.selectFirst(".featured-content-image")
                                                ?: return@mapNotNull null
                                val href =
                                        content.selectFirst("a")?.attr("href")
                                                ?: return@mapNotNull null
                ...
                val hasNext =
                        document.select("ul.pagination").let { pagination ->
                                pagination
                                        ?.last()
                                        ?.select("li:last-child a:contains(Next)")
                                        ?.firstOrNull()
                                        ?.let {
        ...
        private suspend fun handleNormalPage(
                page: Int,
                request: MainPageRequest
        ): HomePageResponse {
                val targetUrl = "${request.data}/page/${page}/"
        ...
        override suspend fun search(query: String): List<SearchResponse> {
                ...
                return document.select(".video-item").mapNotNull {
                        val href =
                                it.selectFirst("a")?.attr("href")?.trim() ?: return@mapNotNull null
        ...
        override suspend fun load(url: String): LoadResponse {
                ...
                val poster =
                        videoPlayer
                                .select("meta[itemprop=thumbnailUrl]")
                                .attr("content")
                                .trim()
                                .ifEmpty {
                                        document.selectFirst(".featured-image img")
                                                ?.attr("src")
                                                ?.trim()
                                }
                ...
                val videoUrl =
                        document.selectFirst(".video-player iframe")?.attr("src")?.trim() ?: ""
        ...
        override suspend fun loadLinks(
                ...
                try {
                        ...
                        println(
                                "DEBUG: Manual check and webresolver not found try using loadExtractor"
                        )
        ...
        //         // val jwConfig = Regex("jwplayer\\(.*?\\)\\.setup\\(\\s*(\\{.*?\\})\\s*\\)",
        ...
        //         val masterUrl =
        // Regex("""sources:\[\{\s*file:\s*["'](https?://[^"']+\.m3u8[^"']*)["']""")
        //             .find(unPacked)
        //             ?.groupValues?.get(1)
        ...
        //         // val queryParams =
        // Regex("""master\.m3u8\?(.*?)['"]""").find(scriptContent)?.groupValues?.get(1)
        ...
        //         // val masterUrl =
        // "https://vuvabh8vnota.cdn-centaurus.com/hls2/01/09302/${videoId}_n/master.m3u8?$queryParams"
        //         val iframeDoc = app.get(iframeUrl).document
        //         val scriptContent = iframeDoc.select("script:containsData(sources)").html()
        //         println("DEBUG - JWPlayer Script Content: $scriptContent")

        //         val masterUrl =
        // Regex("""file:"(https://vuvabh8vnota\.cdn-centaurus\.com/hls2/01/09302/[^"]+)""")
